...
function tocarSom(nomeAudio) {
   let som = new Audio(`./src/sons/${nomeAudio}.m4a`);
   som.volume = 0.2;
   som.play();
}
...
function mudarVida() {
   ...
   if (vida < 0 && contador <=0) {
      alert(` Você marcou ${pontos} pontos no total.\n Reinicie a página e continue acertando o Ralph!`)
...
function main() {
   alert('ACERTE O RALPH PARA MARCAR PONTOS')
   addListenerHitBox()
   mudarVida()
}
